...
function build(output = path.join(__dirname, '..', 'public'), env = process.env) {
  ...
  for (const file of fs.readdirSync(root).filter(name => /\.(html|xml|txt)$/.test(name))) {
    ...
    if (enabled && file === 'privacy.html') content = content.replace('Ads are currently disabled until our publisher account is configured.', 'Google AdSense advertising is enabled on this site.');

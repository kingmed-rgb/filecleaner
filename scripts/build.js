...
const { publisherId, enabled: defaultEnabled } = require('../adsense.json');
...
function build(output = path.join(__dirname, '..', 'public'), env = process.env) {
  ...
  const enabled = env.ADSENSE_ENABLED == null ? defaultEnabled : env.ADSENSE_ENABLED === 'true';
  ...
  for (const file of fs.readdirSync(root).filter(name => /\.(html|xml|txt|css)$/.test(name))) {
    ...
    if (enabled && file === 'privacy.html') content = content.replace('Ads are currently disabled while site approval and consent setup are completed.', 'The Google AdSense script is enabled on this site.');
